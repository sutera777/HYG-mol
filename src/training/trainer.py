...
def evaluate_regression(model, device, loader, phase="validation"):
    ...
    if not y_pred_list or not y_true_list:
        ...
        return 0.0, float('inf'), float('inf'), -float('inf') # MSE=0, RMSE/MAE=inf, R2=-inf
    try:
 
        y_pred = torch.cat(y_pred_list, dim=0).numpy() # Shape: [total_samples, num_targets]
        y_true = torch.cat(y_true_list, dim=0).numpy() # Shape: [total_samples, num_targets]
