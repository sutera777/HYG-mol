# src/explainability/analyzer.py
import os
import base64
...
import io
from ..data.dataset import MoleculeHypergraph, MoleculeData,MoleculeDataset

class MolecularExplainabilityAnalyzer:

    def __init__(self, model, device: str = "cpu", dataset: Optional['MoleculeDataset'] = None, feature_type: str = 'combined', verbose: bool = True):
        self.model = model
        self.device = device
    ...
    def analyze_prediction(self, mol_data, mol_smiles, true_value=None, task_index_to_analyze: int = 0): 
        ...
        with torch.no_grad():
            ...
            if "Attention" in model_class_name:
 
                full_prediction_output = self.model(
                    x, edge_index, batch,
