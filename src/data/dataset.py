...
from rdkit.Chem import Draw, AllChem, rdMolDescriptors, Descriptors, Crippen, MolSurf
...
class MoleculeHypergraph:
    def __init__(self, feature_type):
        ...
        if self.feature_type != "traditional_only": 
            self.tokenizer = AutoTokenizer.from_pretrained(config.CHEMBERTA_MODEL_NAME) 
 
            self.chemberta_model = AutoModel.from_pretrained(config.CHEMBERTA_MODEL_NAME).to('cpu')
        else:
            self.tokenizer = None
        ...
        for i, (edge, label) in enumerate(zip(self.hyperedges, self.hyperedge_labels)):
            ...
            for atom_idx in edge:
                if atom_idx < self.mol.GetNumAtoms():
                    ...
                    if atomic_num in [7, 8, 9, 17, 35, 53]:  # N, O, F, Cl, Br, I
...
class MoleculeDataset(Dataset):
    def __init__(self, data_path, dataset_name, task_type='classification', 
                 max_samples=None, feature_type='combined'):
        ...
        self.cache_dir = f"./processed_data/{dataset_name}/"
        ...
        for i, smiles in enumerate(tqdm(self.smiles_list)):
            if pd.isna(smiles):
                ...
                continue
            try:
                mol = self._robust_smiles_to_mol(smiles) 
                if mol is None:
                    error_count += 1
                    continue
                hypergraph = MoleculeHypergraph(feature_type=self.feature_type) 
                try:
                ...
                if hyperedge_attr is None:
                    ...
                    hyperedge_attr = np.zeros((1, 5), dtype=np.float32)  
    ...
    def _robust_smiles_to_mol(self, smiles):
            ...
            if self.is_multi_label:
                ...
                for task_idx in range(num_tasks):
                    ...
                    if not task_labels:
                        ...
                        invalid_tasks.append((self.label_cols[task_idx], [])) 
                        continue
    ...
    def _save_metadata(self):
        ...
        metadata = {
            ...
            'label_encoding': getattr(self, 'label_encoding', {})  
        }
