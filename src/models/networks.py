...
class CustomHypergraphConv(nn.Module):
    ...
    def forward(self, x, hyperedge_index, hyperedge_attr=None,
        ...
        node_feats = self.node_lin(x).view(-1, self.heads, self.out_channels)  # [N, heads, C_per_head]
        edge_feats = self.edge_lin(hyperedge_attr).view(-1, self.heads,
                                                        self.out_channels)  # [num_edges, heads, C_per_head]
        alpha_l = (node_feats * self.attn_l).sum(dim=-1)  # [N, heads]
        alpha_r = (edge_feats * self.attn_r).sum(dim=-1)  # [num_edges, heads]
        alpha_l_selected = alpha_l[node_idx]  # [E_incidences, heads]
        alpha_r_selected = alpha_r[edge_idx_incidence]  # [E_incidences, heads]
        alpha = alpha_l_selected + alpha_r_selected  # [E_incidences, heads]
        alpha = pyg_softmax(alpha, edge_idx_incidence)  
        alpha = F.leaky_relu(alpha, self.negative_slope)
        alpha = self.dropout(alpha)
        m_node2edge = node_feats[node_idx] * alpha.unsqueeze(-1)  # [E_incidences, heads, C_per_head]
        edge_aggr = scatter_add(m_node2edge, edge_idx_incidence, dim=0,
                                dim_size=num_edges)  # [num_edges, heads, C_per_head]
        edge_aggr = edge_aggr + edge_feats  
        m_edge2node = edge_aggr[edge_idx_incidence] * alpha.unsqueeze(-1)  # [E_incidences, heads, C_per_head]
 
        node_aggr = scatter_add(m_edge2node, node_idx, dim=0, dim_size=num_nodes)  # [N, heads, C_per_head]
...
class AttentionHyperGraphNet(nn.Module):
    def __init__(self, num_node_features, hidden_channels, num_classes,
                 ...
                 attention_mode='node', heads=4, dropout=0.2, hyperedge_dim=5):
        super().__init__()
        self.task_type = task_type
        self.is_multi_label = is_multi_label
        self.hidden_channels = hidden_channels
        self.hyperedge_encoder = nn.Sequential(
            nn.Linear(config.HYPEREDGE_INPUT_DIM, hidden_channels),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
    ...
    def forward(self, x, hyperedge_index, batch, store_attention=False, hyperedge_attr=None):
        ...
        if hyperedge_attr is None:
            ...
            hyperedge_attr = torch.zeros((num_edges, config.HYPEREDGE_INPUT_DIM), device=x.device)
        else:
